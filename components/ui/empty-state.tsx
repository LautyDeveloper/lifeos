...
export function EmptyState({ icon: Icon, title, description, action }: {
  ...
  return (
    <div className="py-14 text-center">
      ...
      {action ? <Link href={action.href} className="mt-6 inline-flex min-h-11 items-center rounded-[18px] border border-primary/15 bg-primary/12 px-4 text-sm font-medium text-primary">{action.label}</Link> : null}
    </div>
  )
}
